...
export default function CheckInPage() {
    ...
    return (
        <div className="h-screen bg-gradient-to-br from-slate-50 via-white to-teal-50/30 flex items-center justify-center p-4 overflow-hidden">
            ...
            <div className="relative w-full max-w-5xl h-full max-h-[88vh] flex flex-col">
                {/* Compact Header */}
                <div className="text-center mb-4 flex-shrink-0">
                    <button onClick={() => router.push('/dashboard')} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:text-teal-600 mb-2">
                        <ArrowLeft size={14} /> Back
                    </button>
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-white rounded-full border border-teal-200/50 mb-2">
                ...
                {/* Main Card - Compact */}
                <div className="relative overflow-hidden rounded-2xl bg-white border border-slate-200/50 shadow-lg p-5 flex-1 flex flex-col min-h-0">
                    <AnimatePresence mode="wait">
                        {/* Step 0 */}
                        {currentStep === 0 && (
                            <motion.div key="step0" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="flex-1 flex flex-col min-h-0">
                                ...
                                <div className="flex-1 grid grid-cols-2 lg:grid-cols-3 gap-3 content-start overflow-auto">
                                    {SYMPTOMS.map((symptom) => (
                                        <div key={symptom.id} className="bg-slate-50/50 rounded-xl p-3 border border-slate-200/50">
                        ...
                        {/* Step 1 */}
                        {currentStep === 1 && (
                            <motion.div key="step1" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="flex-1 flex flex-col justify-center">
                                    ...
                                    {['Yes', 'Partially', 'Missed'].map((option) => (
                                        <button key={option} type="button" onClick={() => setFormData(p => ({ ...p, medication: option }))}
                                            className={cn("py-5 px-4 rounded-xl border-2 font-bold transition-all",
                                                formData.medication === option ? "border-teal-600 bg-teal-50 shadow-lg shadow-teal-600/20 scale-105" : "border-slate-200 bg-white hover:border-teal-300")}>
                        ...
                        {/* Step 2 */}
                        {currentStep === 2 && (
                            <motion.div key="step2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="flex-1 flex flex-col justify-center">
                                <div className="text-center mb-6">
                                    <div className="p-4 bg-teal-100 rounded-full w-fit mx-auto mb-3">
                    ...
                    {/* Navigation */}
                    <div className="flex items-center justify-between pt-3 border-t border-slate-200/50 mt-3 flex-shrink-0">
                        <button onClick={() => setCurrentStep(Math.max(0, currentStep - 1))} disabled={currentStep === 0}
                            className={cn("flex items-center gap-1 px-3 py-2 rounded-lg font-semibold text-xs", currentStep === 0 ? "opacity-0 pointer-events-none" : "bg-slate-100 text-slate-700 hover:bg-slate-200")}>
                            <ChevronLeft size={16} /> Back
                        </button>
                        <div className="flex gap-1.5">
                            {[0, 1, 2].map((step) => (
                                <div key={step} className={cn("w-1.5 h-1.5 rounded-full transition-all", step === currentStep ? "bg-teal-600 w-5" : "bg-slate-300")} />
                            ))}
                        </div>
                        {currentStep < totalSteps - 1 ? (
                            <button onClick={() => setCurrentStep(currentStep + 1)} className="flex items-center gap-1 px-3 py-2 bg-gradient-to-r from-teal-600 to-teal-500 text-white font-semibold text-xs rounded-lg">
                                Next <ChevronRight size={16} />
                            </button>
                        ) : (
                            <button onClick={handleSubmit} disabled={loading} className="flex items-center gap-1 px-5 py-2 bg-gradient-to-r from-teal-600 to-teal-500 text-white font-semibold text-xs rounded-lg disabled:opacity-50">
                                {loading ? (isEditing ? 'Updating...' : 'Submitting...') : (isEditing ? 'Update' : 'Complete')} <CheckCircle2 size={16} />
                            </button>
                        )}
                    </div>
                </div>
